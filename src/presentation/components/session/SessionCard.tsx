...
export function SessionCard({ session, viewAs, currentUserId }: Props) {
  ...
  // track rated state locally so star UI hides after submit
  ...
  async function confirmCompletion() {
    ...
    try {
      ...
      // API tru1ea3 vu1ec1 { success, session: result, message }
  ...
  async function markNoShow() {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/sessions/${session.id}/no-show`, { method: "POST" });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
      // API tru1ea3 vu1ec1 { success, session: result, message }
      const newStatus = data.session?.status;
  ...
  // Gu1ecdi u0111u00fang endpoint /api/payments/session-fee vu1edbi sessionId trong body
  ...
  async function confirmCancel() {
    ...
    await doAction("cancel", { cancelReason: cancelReason.trim() || "u00dd do" });
  }
  ...
  async function submitReport() {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
      toast.success("u0110u00e3 gu1eed bu00e1o cu00e1o. Cu1ea3m u01a1n bu1ea1n!");
      setReported(true);
      setShowReport(false);
    } catch (err: unknown) {
      toast.error(err instanceof Error ? err.message : "Cu00f3 lu1ed7i xu1ea3y ra");
    } finally {
  ...
  return (
    <>
      <div className="bg-white rounded-2xl border border-stone-100 shadow-sm hover:shadow-md transition-all overflow-hidden">
        ...
        {expanded && (
          <div className="px-5 pb-4 border-t border-stone-50 pt-4 space-y-3">
            ...
            {/* u0110u00e1nh giu00e1 Mentor u2014 chu1ec9 hiu1ec7n khi chu01b0a u0111u00e1nh giu00e1 */}
        ...
        <div className="px-5 py-3 bg-stone-50/60 border-t border-stone-50 flex items-center gap-2 flex-wrap">
          ...
          {/* Mentor: xu00e1c nhu1eadn u0111u1eb7t lu1ecbch (PENDING u2192 CONFIRMED) */}
          ...
          {/* Mentor: xu00e1c nhu1eadn hou00e0n thu00e0nh + vu1eafng mu1eb7t */}
          {viewAs === "mentor" &&
          ...
          {/* Mentee: xu00e1c nhu1eadn u0111u00e3 hu1ecdc xong */}
          {viewAs === "mentee" &&
            (localStatus === "CONFIRMED" || localStatus === "IN_PROGRESS") &&
            !menteeConfirmed && (
              <button
          ...
          {/* Bu00e1o cu00e1o (chu1ec9 sau khi hou00e0n thu00e0nh) */}
          ...
          {/* Huu1ef7 buu1ed5i hu1ecdc u2014 cu00f3 confirm dialog */}
      ...
      {/* Dialog xu00e1c nhu1eadn huu1ef7 */}
      {showCancelConfirm && (
        <div
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div>
                <label className="text-sm font-medium text-stone-700 mb-1.5 block">
                  Lu00fd do huu1ef7{" "}
                  <span className="text-stone-400 font-normal">(tu00f9y chu1ecdn)</span>
                </label>
                <textarea
                  value={cancelReason}
      ...
      {showReport && (
        ...
        >
          <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="px-6 py-5 space-y-4">
              <p className="text-sm text-stone-500">
                Bu00e1o cu00e1o nu00e0y su1ebd u0111u01b0u1ee3c gu1edi u0111u1ebfn quu1ea3n tru1ecb viu00ean u0111u1ec3 xem xu00e9t.
              </p>
              <div>
                <label className="text-sm font-medium text-stone-700 mb-1.5 block">
                  Lu00fd do <span className="text-red-500">*</span>
                </label>
                <select
                  value={reportReason}
              ...
              <div>
                <label className="text-sm font-medium text-stone-700 mb-1.5 block">
                  Mu00f4 tu1ea3 chi tiu1ebft <span className="text-red-500">*</span>
                </label>
                <textarea
            ...
            <div className="px-6 py-4 border-t border-stone-100 flex items-center gap-3 bg-stone-50/60">
              ...
              >
                ...
                {reportLoading ? "u0110ang gu1edi..." : "Gu1edi bu00e1o cu00e1o"}
              </button>
            </div>
          </div>
        </div>
      ...
      {/* Modal thanh tou00e1n hu1ecdc phu00ed */}
